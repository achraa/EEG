...
def differential_entropy_features(data, freq_bands):
    '''
    Menghitung fitur Differential Entropy (DE) untuk setiap pita frekuensi.

    Args:
        data (ndarray): Data EEG dalam format 4D (trials, epochs, channels, samples).
        freq_bands (ndarray): Pita frekuensi yang akan dihitung.

    Returns:
        ndarray: Matriks fitur 2D (jumlah_sampel, jumlah_fitur).
    '''
    # dimensi data
    n_trials, n_secs, n_channels, sfreq = data.shape
    # Jumlah fitur per channel adalah jumlah pita frekuensi yang dihitung
    features_per_channel = len(freq_bands) - 1

    features = np.empty([n_trials, n_secs, n_channels * features_per_channel])

    # iterasi untuk setiap trial dan epoch
    for i, trial in enumerate(data):
        for j, second in enumerate(trial):
            # DE (menggunakan spectral entropy) untuk semua channel di epoch ini
            psd = mne_f.compute_spect_entropy(
                sfreq, data, psd_method='welch', psd_params=None)
            de_features = np.log(psd + 1e-10)

            features[i][j] = de_features

    # 7. Reshape menjadi format 2D (sampel, fitur) untuk model machine learning
    features = features.reshape(
        [n_trials * n_secs, n_channels * features_per_channel])
